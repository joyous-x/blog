...
- 案例: 业务消息的可靠传输
    + 方案环评
        ![360环评](../images/arch_compare_360_degrees.png)
    + 结论：最终选择备选方案 2，原因有：
...
- FMEA实践
    ...
    + FMEA 分析后，能够有什么样的发现，下表是分析的样例
        ![FMEA 分析实践](../images/arch_FEMA_analyze_sample.png)
...
- SOA(Service Oriented Architecture)
    ![SOA架构](../images/arch_SOA_architecture.png)
...
- 拆分方法
    ...
    + 基于性能拆分
- 基础设施
    ![微服务基础设施](../images/arch_meshsrv_ infrastructure.png)
- 思考题
...
![互联网技术演进的模式](../images/arch_internet_company_envolution.png)

<h1 id="40" ></h1>

[_页首_](#h)
## 40 | 互联网架构模板：“存储层”技术
![互联网标准架构图](../images/arch_internet_arch_sample.png)
- 互联网架构模板中的存储层技术
    + SQL
        - 单表 
        - 分库分表 
    ...
    ![淘宝TFS架构](../images/arch_taobao_TFS_architecture.png)
- 思考题
...
- 服务层技术：服务层的主要目标其实就是为了降低系统间相互关联的复杂度。
    ...
    + 服务中心
        - 两种实现方式：服务名字系统（Service Name System） 和 服务总线系统（Service Bus System）
            ![服务名字系统](../images/arch_srvcenter_namesystem.png)
            ![服务总线系统](../images/arch_srvcenter_bussystem.png)
            ![服务总线、名字系统对比](../images/arch_srvcenter_name_bus_compare.png)
    + 消息队列
- 思考题
...
- 测试平台
    ...
    ![测试平台基本架构](../images/arch_test_platform_sample.png)
    + 用例管理
