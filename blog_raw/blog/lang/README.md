---
published: true
title: Language 目录
description: 编程语言相关知识汇总
keywords:
- lang
categories:
- lang
permalink: /blog/lang/
date: 2020-04-13
tags:
- null
---

# LANGUAGE 目录

## 一、 Cxx
* [只能分配在堆或栈上的类](../lang/cxx/class_only_on_head_and_stack.md)
* [Const详解](../lang/cxx/const_details.md)

## 二、 Go
* [Go Mod 简介](../lang/go/gomod.md)
* [Go 知识点](../lang/go/go_utils.md)
* [Recover & Const 简述](../lang/go/recover&const_desc.md)
* [Slice 底层结构](../lang/go/slice_base.md)
* [关于 Nil 判定的一些事情](../lang/go/somethings_about_nil.md)

## 三、 Js
* [Js 零基础起步](../lang/js/js_zero_start.md)

## 四、 Python
* [Python 环境问题记录](../lang/python/py_env.md)
* [Python 中 "\_"、"\_\_" 和 "\_\_Xx\_\_" 的区别](../lang/python/py__xx__.md)
* [Python 基础](../lang/python/utils.md)
