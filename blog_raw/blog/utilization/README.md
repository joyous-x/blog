---
published: true
title: utilization
description: jiao's blog
keywords:
- null
categories:
- utilization
- devops
permalink: /blog/utilization/
date: 2020-04-13
tags:
- null
---

# UTILIZATION

* [汉字字符编码及转换](../utilization/ch_pinyin_tran.md)
* [反编译Apk](../utilization/foo_apk_reverse.md)
* [实践：批量拉取Git项目脚本](../utilization/foo_batch_pulls.md)
* [实践：Consul 部署](../utilization/foo_consul.md)
* [实践：Dind](../utilization/foo_docker_jenkins_dind.md)
* [实践：K8S 之 Downward Api](../utilization/foo_k8s_downward_api.md)
* [实践：创建Redis Cluster](../utilization/foo_redis_cluster.md)