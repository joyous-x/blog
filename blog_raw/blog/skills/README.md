---
published: true
title: Skills 目录
description: devops、bigdata、ml、ai 等前沿技术技能
keywords:
- ''
categories:
- skills
- devops
- image processing
- k8s
permalink: /blog/skills/
date: 2020-04-13 00:00:00
tags:
- null
---

# SKILLS 目录

## 一、 Ai
* [漫步神经网络应用](../skills/ai/anime_roam.md)
* [机器学习评估指标:分类](../skills/ai/auc_roc.md)
* [大数据架构理论](../skills/ai/bigdata_arch_theory.md)
* [Pca原理推导](../skills/ai/derivation_of_PCA.md)
* [Nn 论文阅读](../skills/ai/image_papers.md)
* [图像处理基础](../skills/ai/imgproc_base.md)
* [Pytorch入门(快速、深度)](../skills/ai/pytorch_base.md)
* [Single Image Super Resolution](../skills/ai/super_resolution.md)

## 二、 Devops
* [K8S网络之网络框架](../skills/devops/k8s_net_mode.md)
* [K8S网络之Service间的通信](../skills/devops/k8s_net_srv.md)
* [K8S 的 Rolling Update](../skills/devops/k8s_rolling_update.md)
* [Opentracing 简介](../skills/devops/opentracing_overview.md)
* [Docker 基础知识](../skills/devops/docker-base.md)
* [K8S-Ipvs连接保持](../skills/devops/ipvs_in_k8s.md)
* [Kubernetes 简述](../skills/devops/k8s_base.md)
* [K8S网络之集群外访问Service的方式](../skills/devops/k8s_net_expose.md)
